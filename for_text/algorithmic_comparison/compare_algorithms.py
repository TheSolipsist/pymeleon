...
n = 100
n_scen = 5
for fitness_str in ("neural_random", "heuristic"):
    for mod in modules:
        print(fitness_str, mod.__name__)
        found = [0] * n_scen
        ...
        with open(f"results/experiments/CGP/{fitness_str}_{mod.__name__}", "w") as f:
            f.write(f"THIS IS {mod.__name__}\n\n")
            for i in range(n_scen):
                f.write(f"Scenario {i + 1}:\n")
                f.write(f"\tAccuracy: {found[i] / n}\n")
                f.write(f"\tIterations: {total_iterations[i] / max(found[i], 1)}\n")
                f.write(f"\tTime: {total_times[i] / max(found[i], 1)}\n")
